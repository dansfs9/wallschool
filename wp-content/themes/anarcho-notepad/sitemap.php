...
<div class="html-sitemap">
   <!-- <h2><?php _e('Autor(es):', 'anarcho-notepad'); ?></h2>
    ...
    <h2><?php _e('Páginas:', 'anarcho-notepad'); ?></h2>
    ...
    foreach ($cats as $cat) {
      echo '<li class="category">'."\n".'<h3><span class="grey"><?php _e('Categoria: ', 'anarcho-notepad'); ?></span>'.$cat->cat_name.'</h3>'."\n";
      ...
      while ($query->have_posts()): $query->the_post(); 
         ...
         if ($category[0]->cat_ID == $cat->cat_ID) {?>
            <li><?php the_time('M d, Y')?> &raquo; <a href="<?php the_permalink() ?>"  title="Link para: <?php the_title(); ?>">
    ...
    wp_reset_query(); 
    ?>   
    <h2><?php _e('Arquivos:', 'anarcho-notepad'); ?></h2>   
