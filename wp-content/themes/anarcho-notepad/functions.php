...
function anarcho_breadcrumbs() {
 ...
 if ($thisCat->parent != 0) echo (get_category_parents($parentCat, TRUE, ' ' . $delimiter . ' ')) ;
	echo $before . __('Arquivados por categoria ', 'anarcho-notepad') . '"' . single_cat_title('', false) . '"' . $after;
 } elseif ( is_day() ) {
	echo '<a href="' . get_year_link(get_the_time('Y')) . '">' . get_the_time('Y') . '</a> ' . $delimiter . ' ';
	echo '<a href="' . get_month_link(get_the_time('Y'),get_the_time('m')) . '">' . get_the_time('F') . '</a> ' . $delimiter . ' ';
	echo $before . __('Arquivados por data ', 'anarcho-notepad') . '"' . get_the_time('d') . '"' . $after;
 } elseif ( is_month() ) {
	echo '<a href="' . get_year_link(get_the_time('Y')) . '">' . get_the_time('Y') . '</a> ' . $delimiter . ' ';
	echo $before . __('Arquivados por mês ', 'anarcho-notepad') . '"' . get_the_time('F') . '"' . $after;
 } elseif ( is_year() ) {
	echo $before . __('Arquivados por ano ', 'anarcho-notepad') . '"' . get_the_time('Y') . '"' . $after;
 ...
 if ( get_post_type() != 'post' ) {
	...
	echo $before . __('Você está lendo ', 'anarcho-notepad') . '"' . get_the_title() . '"' .  $after;
 }
/* } elseif ( !is_single() && !is_page() && get_post_type() != 'post' && !is_404() ) {
	...
	echo $before . 'Você está vendo "' . get_the_title() . '"' . $after;
 } elseif ( is_page() && !$post->post_parent ) {
	echo $before . 'Você está lendo "' . get_the_title() . '"' . $after;
 } elseif ( is_page() && $post->post_parent ) {
	$parent_id  = $post->post_parent;
	$breadcrumbs = array();
	while ($parent_id) {
	...
	echo $before . __('Procurar resultados para ', 'anarcho-notepad') . '"' . get_search_query() . '"' . $after;
 } elseif ( is_tag() ) {
	echo $before . __('Arquivo por tag ', 'anarcho-notepad') . '"' . single_tag_title('', false) . '"' . $after;
 } elseif ( is_author() ) {
	global $author;
	$userdata = get_userdata($author);
	echo $before . __('Artigos postados por ', 'anarcho-notepad') . '"' . $userdata->display_name . '"' . $after;
 } elseif ( is_404() ) {
...
function anarcho_page_nav() {
 if(get_theme_mod('enable_page-nav') == '1') {
  ...
  $a['mid_size'] = 3;
  $a['end_size'] = 1;
  $a['prev_text'] = __('Página Anterios', 'anarcho-notepad');
  $a['next_text'] = __('Próxima Página', 'anarcho-notepad');
  ...
  if ($max > 0) echo '</nav><br/>';
	...
	<nav class="navigation paging-navigation" role="navigation">
		<h1 class="screen-reader-text"><?php _e( 'Navegação pelos Posts', 'anarcho-notepad' ); ?></h1>
		<div class="nav-links">

			<?php if ( get_next_posts_link() ) : ?>
			<div class="nav-previous"><?php next_posts_link( '<i class="fa fa-arrow-left"></i> Posts Antigos' ); ?></div>
			<?php endif; ?>

			<?php if ( get_previous_posts_link() ) : ?>
			<div class="nav-next"><?php previous_posts_link( 'Posts Recentes <i class="fa fa-arrow-right"></i>' ); ?></div>
			<?php endif; ?>
...
function anarcho_comment( $comment, $args, $depth ) {
	...
	<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
		<p><?php _e( 'Pingback:', 'anarcho-notepad' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( __( '(Editar)', 'anarcho-notepad' ), '<span class="edit-link">', '</span>' ); ?></p>
	<?php
			break;
		default :
	...
	<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
		<article id="comment-<?php comment_ID(); ?>" class="comment">
			<header class="comment-meta comment-author vcard">
				<?php
					...
					printf( '<cite><b class="fn">%1$s</b> %2$s</cite>',
						...
						( $comment->user_id === $post->post_author ) ? '<span>' . __( 'Autor do Post', 'anarcho-notepad' ) . '</span>' : ''
					);
			...
			<?php if ( '0' == $comment->comment_approved ) : ?>
				<p class="comment-awaiting-moderation"><?php _e( 'Seu comentário será publicado em breve.', 'anarcho-notepad' ); ?></p>
			<?php endif; ?>

			<section class="comment-content comment">
				<?php comment_text(); ?>
				<?php edit_comment_link( __( 'Editar', 'anarcho-notepad' ), '<p class="edit-link">', '</p>' ); ?>
			</section><!-- .comment-content -->

			<div class="reply">
				<?php comment_reply_link( array_merge( $args, array( 'reply_text' => __( 'Responder', 'anarcho-notepad' ), 'after' => ' <span>&darr;</span>', 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?>
			</div><!-- .reply -->
		</article><!-- #comment-## -->
	<?php
		break;
...
function anarcho_comment_form($anarcho_defaults) {
	...
	$anarcho_fields = array(
		'author' => '<p class="comment-form-author">' . '<label for="author">' . __( 'Nome:', 'anarcho-notepad' ) . '</label> ' . ( $req ? '<span class="required">*</span>' : '' ) .
		            '<input id="author" name="author" placeholder="' . __('Nome', 'anarcho-notepad') . '" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' /></p>',
	...
	$anarcho_defaults['comment_field'] = '<p><label for=comment">' . _x( 'Comentário:', 'noun', 'anarcho-notepad' ) . '</label><textarea id="comment" name="comment" placeholder="' . __('Comentário', 'anarcho-notepad') . '" cols="45" rows="4" aria-required="true"></textarea></p>';

	$anarcho_defaults['title_reply'] = __('Deixe um Comentário', 'anarcho-notepad');
